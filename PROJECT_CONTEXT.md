...
- Mercado Pago Pix now also has an active reconciliation fallback on read paths: public confirmation, admin order lists, and KDS can poll the payment safely for pending Pix charges, so a paid order can self-heal even if the webhook arrives late or is temporarily unavailable.
...
- Stock commitment for public availability now follows a stricter rule: any common order reserves stock as soon as the store moves it out of `FEITO` (for example on `ACEITO`), independent of payment method, and delivery is still the moment of definitive baixa; `ENCOMENDA` keeps reserving on production-oriented statuses (`PREPARACAO` / `PRONTO_ENTREGA`).
...
- The restricted login uses the official Brookie background and mark, and the browser/PWA identity uses the same official brand asset. `/admin` goes directly to the login flow, which separates password and authenticator into two screens. The master can invite named administrators from the security center; the link is single-use and activation finishes only after password and TOTP enrollment.
- Production admin invitations now fail closed before persistence when transactional e-mail is unavailable, enforce the configured admin e-mail allowlist server-side, and revoke plus audit delivery failures without exposing the one-time link.
- Invite completion now shares the same credential policy between browser and server, shows each password requirement before submission, returns field-specific validation feedback and keeps the card/input colors theme-aware so dark-mode mobile devices retain readable contrast.
