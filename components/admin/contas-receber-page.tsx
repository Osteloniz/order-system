...
import { BadgeDollarSign, CreditCard, RefreshCw, Search, Wallet } from 'lucide-react'
...
export function ContasReceberPage() {
  ...
  const resumoBusca = useMemo(() => contasFiltradas.reduce((acc, conta) => {
    acc.total += conta.total
    acc.taxa += conta.taxa
    acc.liquido += conta.liquido
    return acc
  }, { total: 0, taxa: 0, liquido: 0 }), [contasFiltradas])
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold"><BadgeDollarSign className="h-6 w-6 text-primary" />Contas a receber</h1>
          <p className="mt-1 text-sm text-muted-foreground">Veja o que ainda vai entrar, o que ja entrou e o impacto das taxas de cartao.</p>
        </div>
        <div className="grid gap-2 md:grid-cols-[180px_180px_180px_auto_auto]">
          ...
          <div className="space-y-2">
            ...
            <select value={statusInput} onChange={(event) => setStatusInput(event.target.value as typeof statusInput)} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Previsto</p><p className="mt-1 text-3xl font-bold">{formatarMoeda(data?.resumo.previsto ?? 0)}</p><p className="mt-2 text-xs text-muted-foreground">Liquido: {formatarMoeda(data?.resumo.previstoLiquido ?? 0)}</p></CardContent></Card>
          <Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Realizado</p><p className="mt-1 text-3xl font-bold">{formatarMoeda(data?.resumo.realizado ?? 0)}</p><p className="mt-2 text-xs text-muted-foreground">Liquido: {formatarMoeda(data?.resumo.realizadoLiquido ?? 0)}</p></CardContent></Card>
          <Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Taxas do periodo</p><p className="mt-1 text-3xl font-bold">{formatarMoeda(data?.resumo.totalTaxas ?? 0)}</p><p className="mt-2 text-xs text-muted-foreground">Cartao credito e debito</p></CardContent></Card>
          <Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Cancelado</p><p className="mt-1 text-3xl font-bold">{formatarMoeda(data?.resumo.cancelado ?? 0)}</p><p className="mt-2 text-xs text-muted-foreground">{data?.totalRegistros ?? 0} registro(s)</p></CardContent></Card>
        </div>
      )}
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="relative w-full md:max-w-sm">
            ...
            <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por pedido, cliente ou pagamento" className="pl-9" />
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl border bg-muted/20 p-4"><p className="text-sm text-muted-foreground">Bruto filtrado</p><p className="mt-1 text-2xl font-bold">{formatarMoeda(resumoBusca.total)}</p></div>
            <div className="rounded-xl border bg-muted/20 p-4"><p className="text-sm text-muted-foreground">Taxa filtrada</p><p className="mt-1 text-2xl font-bold">{formatarMoeda(resumoBusca.taxa)}</p></div>
            <div className="rounded-xl border bg-muted/20 p-4"><p className="text-sm text-muted-foreground">Liquido filtrado</p><p className="mt-1 text-2xl font-bold">{formatarMoeda(resumoBusca.liquido)}</p></div>
          </div>
          ...
          {isLoading ? (
                ...
                <tbody>
                  {contasFiltradas.map((conta) => (
                    <tr key={conta.id} className="border-t odd:bg-background even:bg-muted/15">
                      ...
                      <td className="py-3 pr-4"><div><p className="font-medium">{conta.clienteNome}</p><p className="text-xs text-muted-foreground">{formatarTelefone(conta.clienteTelefone)}</p></div></td>
            ...
            <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">Nenhum recebimento encontrado para os filtros aplicados.</div>
          )}
        </CardContent>
      </Card>
    </div>
