...
import type { Cliente, Cupom, Pedido, Produto, SeparacaoResponsavelPessoa, TipoCartao, TipoEntrega, TipoPagamento } from '@/lib/types'
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                {clienteId && (
                  <div className="flex flex-col gap-2 rounded-lg border border-primary/25 bg-primary/10 p-3 text-sm text-primary sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-start gap-2">
                      <UserRound className="h-4 w-4" />
                      <span>Cliente vinculado ao cadastro.</span>
                    </div>
                    <Button type="button" variant="ghost" size="sm" className="self-end sm:self-auto" onClick={clearSelectedCliente}>
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
                )}
              ...
              <div className="grid min-w-0 gap-4 sm:grid-cols-2">
                ...
                <div className="min-w-0 space-y-2">
                  ...
                  <select value={pagamento} onChange={event => setPagamento(event.target.value as TipoPagamento)} className="h-9 w-full min-w-0 rounded-md border border-input bg-background px-3 text-sm">
                  ...
                  {pagamento === 'CARTAO' && (
                    <div className="grid gap-2 sm:grid-cols-2">
                      <label className={`flex cursor-pointer items-start gap-3 rounded-md border px-3 py-3 text-sm transition-colors ${tipoCartao === 'CREDITO' ? 'border-primary bg-primary/5' : 'border-input hover:bg-muted/35'}`}>
                        <input
                          type="radio"
                          name="tipo-cartao-admin"
                          value="CREDITO"
                      ...
                      <label className={`flex cursor-pointer items-start gap-3 rounded-md border px-3 py-3 text-sm transition-colors ${tipoCartao === 'DEBITO' ? 'border-primary bg-primary/5' : 'border-input hover:bg-muted/35'}`}>
                        <input
                          type="radio"
                          name="tipo-cartao-admin"
                          value="DEBITO"
                  ...
                  <>
                    ...
                    <div className="min-w-0 space-y-2 sm:col-span-2">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <Button type="button" variant="outline" size="sm" className="w-full sm:w-auto" onClick={addPessoaSeparacao} disabled={!items.length}><Plus className="mr-2 h-4 w-4" />Adicionar pessoa</Button>
                      </div>
                        ...
                        {separacaoResponsavel.map((pessoa, index) => (
                          <div key={`pessoa-${index}`} className="rounded-xl border p-3">
                            <div className="mb-3 flex items-center justify-between gap-3">
                              ...
                              <Button type="button" variant="ghost" size="sm" className="shrink-0" onClick={() => removePessoaSeparacao(index)}><X className="h-4 w-4" /></Button>
                            </div>
          ...
          <CardContent className="space-y-4">
            {items.length === 0 ? <p className="text-sm text-muted-foreground">Nenhum produto adicionado.</p> : items.map(item => (
              <div key={item.produto.id} className="space-y-2">
                <div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="break-words font-medium">{item.produto.nome}</p><p className="text-sm text-muted-foreground">{formatarMoeda(item.produto.preco)} un.</p></div><Badge>{item.quantidade}x</Badge></div>
                <div className="grid grid-cols-2 gap-2"><Button variant="outline" size="sm" className="w-full" onClick={() => changeQuantity(item.produto.id, -1)}><Minus className="mr-2 h-4 w-4" />Remover</Button><Button variant="outline" size="sm" className="w-full" onClick={() => changeQuantity(item.produto.id, 1)}><Plus className="mr-2 h-4 w-4" />Adicionar</Button></div>
              </div>
            ))}
            <Separator />
